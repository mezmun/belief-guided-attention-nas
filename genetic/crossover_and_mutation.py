Pasted text.txt
Document
herneyse. mevcut işi iptal edip zamanını 30 dk ya çekmek istiyorım. aşağıda sana slurm dosya içeriğini veriyorum. 


Pasted text.txt
Document
kodda bir süre time sleep var. bunlar içinbirşey demedin


Pasted text.txt
Document
kod .alai;iyor gayet iyi ancak hala 2 model tek jenerasyon eğitim yapıp bitiyor. bunu çözemedim
...
Document
Today 1:24 PM
attention-nas-main (3).zip
Zip Archive
ilk hata ile ilgili bir eklenti yapmamıştım. sadece tekrar çalıştırdım hata tekrar ortaya çıkacak mı diye. benim anlamadığım ben çok kez kodu çalıştırdım bu hataları vermiyordu. sonradan ortaya çıktı bu hatalar. kodun o kısmında değişiklik yapmadım. bu kodlar çok eski ve tekrar tekrar kullanılan kodlar. başkalarıda kullandı bu kodları. bir yerde yanlışlıkla bir hata  ile bir yeri mi sildim veya bir satır falan mı kaydırdım anlamadım. bu gözle kodları inceler misin bu son iki hataya sebep olan yerler üzerinden. sana bu kodun eski versiyonunu vereceğim. oradan bu kısımları github reposunda ki halleriyle karşılaştırır mısın. acaba o kısımları etkileyecek bir eklenti mi yaptım yanlışlıkla. şimdi zip olarak sna veriyorum eski versiyonunu bu kodun. 
Worked for 41s

Haklısınız. Burada yalnızca tek dosyadaki aynı hatalı DenseNet kanal hesabını düzeltmek yeterli. Başka dosyalara, cache sistemine veya model şablonuna dokunmayalım.

Değiştirilecek tek dosya
genetic/crossover_and_mutation.py

Dosyada şu hatalı mantık toplam 6 yerde tekrarlanıyor:
...
if estimated_out_channel > unit.out_channel:
    break
else:
    unit.out_channel = estimated_out_channel
    keep_out_channel = estimated_out_channel

Burada iki problem var:

DenseNet gerçek girişte max_input_channel sınırı kullanıyor.
Yeni çıkış değeri büyüdüğünde break yapılması kanal aktarımını yarıda kesiyor.

Başlangıç popülasyonu DenseNet çıkışını zaten şu şekilde hesaplıyor:

min(in_channel, max_input_channel) + k * amount

Bu nedenle crossover ve mutation tarafının da aynı hesabı kullanması gerekir.

1. unit_list1[j] bloğu

Yaklaşık 316. satırdaki şu kısmı:
...
unit_list1[j].out_channel = estimated_out_channel
keep_out_channel = estimated_out_channel
2. unit_list2[j] bloğu

Yaklaşık 350. satırdaki bloğu şu şekilde değiştirin:
...
Bu iki yer crossover içindir. Mevcut kodda hatalı hesap ve break iki yavru için ayrı ayrı bulunuyor.

3. Mutation içindeki dört aynı blok

Dosyada şu ifadeyi arayın:

estimated_out_channel = indi.units[i].k * indi.units[i].amount + indi.units[i].in_channel

Güncel kodda yaklaşık şu satırlarda bulunuyor:
...
Bunlar sırasıyla ADD, REMOVE, ResNet ALTER ve DenseNet ALTER sonrasındaki kanal güncellemeleridir.

Bu dört yerde aşağıdaki eski bloğu:
...
Şu kısmı tamamen kaldırıyoruz:

if estimated_out_channel > old_out_channel:
    break

Çünkü bu break, kanal zincirini yanlış yerde durduruyor. Bir DenseNet’in çıkışı değiştiğinde sonraki DenseNet birimine doğru kanal aktarılmaya devam etmelidir.

Bu düzeltme iki hatayı da hedefler:

assert(out.size()[1] == self.out_channel)

hatasında DenseNet’in kayıtlı out_channel değeri düzeltilir.

running_mean should contain 176 elements not 512

hatasında ise sonraki modülün in_channel değerinin eski 512 değerinde kalması engellenir.

Başka değişiklik yapmayın

Bu hata için şu dosyalara dokunmayın:
...
içindeki bu 6 aynı kanal hesaplama bloğunu değiştirin.

Değişiklikten sonra eski bozuk bireylerden devam etmemek için yeni deney başlatın:

is_running = 0

İlk kontrolü epoch=1, pop_size=20, max_gen=3 ile yapmanız yeterlidir.

Pasted text.txt
Document
verdiğin düzelemeler doğru mu emin misin. sana kodu tekrar veriyorum. kodu tekrar inceleyip bana gerekli düzenlemeleri tekrar verir misin


Worked for 2m 33s

Tekrar verdiğiniz dosyanın tamamını inceledim. Önceki “break satırını tamamen kaldırın” önerimi düzeltiyorum. Gereğinden genişti.

En küçük ve doğru çözüm:

DenseNet çıkışını max_input_channel sınırıyla hesaplamak.
Çıkış değişmediyse break yapmak.
Çıkış gerçekten değiştiyse out_channel değerini güncelleyip sonraki DenseNet’e devam etmek.

Verdiğiniz dosyada bu hatalı hesap toplam 6 yerde bulunuyor. Başka dosyaya dokunmanız gerekmiyor.

Neden mevcut hesap yanlış?
...
Fakat gerçek DenseNet, giriş kanalı max_input_channel değerinden büyükse önce 1×1 convolution ile giriş kanalını sınırlandırıyor. Bu nedenle gerçek çıkış:

min(in_channel, max_input_channel) + k * amount

olmalı. Başlangıç popülasyonu oluşturulurken de bu formül kullanılıyor.

1. Crossover — unit_list1[j]

Dosyada şu bloğu bulun:
...
# DenseNet'in gerçek çıkışı değişmediyse sonraki katmanlar
# zaten doğru kanal değerlerine sahiptir.
if estimated_out_channel == unit_list1[j].out_channel:
    break
...
unit_list2[j].out_channel = estimated_out_channel
keep_out_channel = estimated_out_channel
3. Mutation bölümlerindeki dört blok

Aynı dosyada şu hesap dört farklı fonksiyonda bulunuyor:
...
Bulunduğu fonksiyonlar:

do_add_unit_mutation
do_remove_unit_mutation
do_alter_resnet_mutation
do_alter_densenet_mutation

Bu dört yerdeki eski bloğu:
...
indi.units[i].out_channel = estimated_out_channel
keep_out_channel = estimated_out_channel
Neden == olduğunda break yapıyoruz?

Örneğin:

in_channel       = 512
max_input_channel = 128
k × amount        = 48
...
DenseNet’in eski out_channel değeri de 176 ise, giriş 512 olarak değişmiş olsa bile DenseNet sıkıştırma convolution’ı nedeniyle çıkış yine 176 kalır. Bu durumda sonraki katmanların kanal değerlerini değiştirmeye gerek yoktur ve break doğrudur.

Ancak eski çıkış 160, yeni gerçek çıkış 176 ise:

out_channel = 176
keep_out_channel = 176

yapılıp sonraki DenseNet katmanlarına devam edilmelidir.

Mevcut kod ise yalnızca yeni sayı eski sayıdan büyük olduğu için break yapıyor ve gerekli güncellemeyi gerçekleştirmiyor. Güncel dosyada crossover tarafındaki bu davranış açıkça görülüyor.

Kontrol

Değişiklikten sonra şu arama hiçbir aktif sonuç döndürmemeli:

grep -n "if estimated_out_channel >" \
    genetic/crossover_and_mutation.py

Yeni hesap altı kez görünmeli:

grep -n "effective_input_channel = min" \
    genetic/crossover_and_mutation.py

Beklenen sonuç sayısı:
...
Ben aynı değişiklikleri gönderdiğiniz dosyanın bir kopyasına uygulayıp py_compile ile kontrol ettim; sözdizimi geçerli.

Son olarak eski hatalı bireyleri yüklememesi için yeni testte:

is_running = 0

kullanın ve eski populations/mutation_*.txt dosyalarını temizleyin. Bu hata için yalnızca genetic/crossover_and_mutation.py içindeki bu altı blok değiştirilmelidir.

bu kodun tamamını düenleyip bana verir misin crossover_and_mutation.py kısmı için
Worked for 44s

Gönderdiğiniz dosyanın tamamını düzenledim. Yalnızca DenseNet kanal hesabının geçtiği 6 blok değiştirildi; crossover, mutation, ECA seçimleri ve diğer akışlara dokunulmadı.

Yapılan düzeltme:

effective_input_channel = min(
    unit.in_channel,
    unit.max_input_channel
)
...
kullanıldı. Çıkış gerçekten değişmişse out_channel güncellenip kanal aktarımı sonraki DenseNet birimine devam ediyor.

Dosya Python sözdizimi kontrolünden başarıyla geçti:

python -m py_compile crossover_and_mutation.py
OK

crossover_and_mutation.py

GitHub’da mevcut genetic/crossover_and_mutation.py dosyasının yerine bunu koyun. Yeni testte eski hatalı nesillerin yüklenmemesi için is_running = 0 ile temiz çalışma başlatın.
